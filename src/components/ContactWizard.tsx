...
type SoumissionType = '' | 'residentiel' | 'commercial';
...
export default function ContactWizard({ idPrefix }: ContactWizardProps) {
  ...
  function chooseType(type: SoumissionType) {
    setData((d) => ({
      ...d,
      soumissionType: type,
      typeEntree: type === 'commercial' ? 'Multilogement ou commercial' : d.typeEntree,
    }));
    setStepError('');
    setStep(1);
  }
  ...
  const stepNumber = step === 'done' ? 4 : step + 1;
  const id = (name: string) => `${idPrefix}-${name}`;

  return (
    <form id={id('contact-form')} className="wizard-form" onSubmit={handleSubmit} noValidate>
      {step !== 'done' && (
        <>
          <p className="wizard-microcopy">Gratuit · Sans engagement · Réponse en 24 h</p>
          <div className="wizard-progress" role="group" aria-label={`Étape ${stepNumber} sur 4`}>
            {[1, 2, 3, 4].map((n) => (
              <span key={n} className={`wizard-dot${n <= stepNumber ? ' is-done' : ''}`} aria-hidden="true" />
            ))}
            <span className="visually-hidden">Étape {stepNumber} sur 4</span>
          </div>
        </>
      )}
      ...
      {step === 0 && (
        <div className="wizard-step">
          <h3 className="wizard-step-title">Votre soumission est pour...</h3>
          <div className="wizard-type-choices">
            <button type="button" className="wizard-type-choice" onClick={() => chooseType('residentiel')}>
              Particulier
            </button>
            <button type="button" className="wizard-type-choice" onClick={() => chooseType('commercial')}>
              Commercial
            </button>
          </div>
        </div>
      ...
      {step === 1 && (
        <div className="wizard-step">
          ...
          <div className="field">
            <label htmlFor={id('adresse')}>Adresse de la propriété <span className="required">*</span></label>
            <input
              type="text"
              id={id('adresse')}
      ...
      {step === 3 && (
        <div className="wizard-step">
          ...
          <div className="form-row">
            <div className="field">
              <label htmlFor={id('prenom')}>Prénom <span className="required">*</span></label>
              <input
                type="text"
                id={id('prenom')}
            ...
            <div className="field">
              <label htmlFor={id('telephone')}>Téléphone <span className="required">*</span></label>
              <input
                type="tel"
                id={id('telephone')}
                autoComplete="tel"
          ...
          <div className="field">
            <label htmlFor={id('courriel')}>Courriel <span className="required">*</span></label>
            <input
              type="email"
              id={id('courriel')}
              autoComplete="email"
