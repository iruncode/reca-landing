...
## Popup de soumission avec formulaire complet (2026-07-26, branche `optimisation-mobile`)
- Suite au retour de l'utilisateur : l'ancien popup Particulier/Commercial qui fermait puis faisait défiler vers le formulaire en bas de page ne fonctionnait pas ("on ne peut pas savoir si c'est une soumission pour particulier ou commercial" une fois qu'on a scrollé). Remplacé par un popup qui contient **tout le formulaire** et s'ouvre sur place, sans scroll.
- Le wizard de contact (adresse → type d'entrée/services → coordonnées, ex-contenu de `ContactForm.tsx`) a été extrait dans `src/components/ContactWizard.tsx`, réutilisé à la fois par la section de bas de page (`ContactForm.tsx`, inchangée en apparence) et par le nouveau `src/components/SubmissionModal.tsx` (remplace `SubmissionTypeModal.tsx`, supprimé). Une étape 0 "Particulier ou Commercial ?" a été ajoutée en tête du wizard (4 étapes au lieu de 3) — le choix "Commercial" pré-sélectionne "Multilogement ou commercial" à l'étape suivante.
- `ContactWizard` prend une prop `idPrefix` (`"page"` ou `"modal"`) pour préfixer tous ses `id` HTML — **nécessaire** car les deux instances (section de bas de page + popup) sont montées simultanément dans le DOM dès que le popup est ouvert ; sans préfixe, les `id`/`<label htmlFor>` entreraient en collision. Vérifié via Playwright (aucun id dupliqué détecté).
